...
var originPixel = map.getPixelFromCoordinate([0,0]);
...
var Data = {
  ...
  scale: function(item) {
    ...
    res.height = 40000//min(item.height/zoomScale, MAX_HEIGHT);
...
function setZoom(z,mapSize) {
  ZOOM = z;
  MAP_SIZE = mapSize || MAP_TILE_SIZE <<ZOOM;
...
function onZoomEnd(e) {
  isZooming = false;
  var maxExtent = e.target.getView().getProjection().getExtent();
  setZoom(e.zoom,e.target.getPixelFromCoordinate([maxExtent[2],maxExtent[1]])[0] - map.getPixelFromCoordinate([maxExtent[0],maxExtent[3]])[0]);
  Data.update(); // => fadeIn()
  Layers.render();
}
...
var osmb = function(map) {
  this.map = map;
  this.maxExtent = this.map.getView().getProjection().getExtent()//[ -20037508.34, -20037508.34, 20037508.34, 20037508.34]; // MaxExtent of layer
  try {
    this.setMap(map);
    map.addLayer(this);
  } catch (e) {
...
proto.setMap = function(map) {
  ...
  this.on('precompose', function(e) {
    setZoom(map.getView().getZoom(),map.getPixelFromCoordinate([this.maxExtent[2],this.maxExtent[1]])[0] - map.getPixelFromCoordinate([this.maxExtent[0],this.maxExtent[3]])[0]);
    scope.setOrigin();
    Data.resetItems();
    Data.update();
  });
