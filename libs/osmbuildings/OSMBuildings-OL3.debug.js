...
function geoToPixel(lat, lon) {
  var pixel = mapObj.getPixelFromCoordinate([lon,lat])
  return {
    x: ORIGIN_X + pixel[0],
    y: ORIGIN_Y + pixel[1]
};
...
var Data = {
  ...
  set: function(data,dataProjection) {
    this.isStatic = true;
    this.resetItems();
    if( dataProjection && mapObj.getView().getProjection().getCode()!== dataProjection){
      var features = new ol.format.GeoJSON().readFeatures(data,{dataProjection:dataProjection,featureProjection:mapObj.getView().getProjection().getCode()})
      this._staticData = new ol.format.GeoJSON().writeFeaturesObject(features);
    }else{
...
function setZoom(z) {
  ZOOM = z;
  var maxExtent = mapObj.getView().getProjection().getExtent();
  MAP_SIZE = mapObj.getPixelFromCoordinate([maxExtent[2],maxExtent[1]])[0] - mapObj.getPixelFromCoordinate([maxExtent[0],maxExtent[3]])[0];
...
var osmb = function(map) {
  this.map = map;
  this.maxExtent = map.getView().getProjection().getExtent();//[ -20037508.34, -20037508.34, 20037508.34, 20037508.34]; // MaxExtent of layer
  try {
    this.setMap(map);
    map.addLayer(this);
  } catch (e) {
...
proto.set = function(data,dataProjection) {
  Data.set(data,dataProjection);
  return this;
};
